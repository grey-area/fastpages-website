...
def write_generated_gitignore(posts_dir: Path, generated: list[str]) -> None:
    """Ignore exactly the posts we generated, and nothing else.

    _posts/ holds both converter output and any hand-written markdown post, so a
    blanket `_posts/*.md` rule would silently ignore a hand-written one -- it
    would never be committed, so CI would never see it and it would never
    publish. Naming the generated files individually keeps them out of git while
    leaving everything else tracked normally. The file ignores itself too, so it
    does not need committing.
    """
    lines = [
        "# Generated by _action_files/nb2post.py -- do not commit, do not edit.",
        "# Only the files below are converter output; any other .md here is a",
        "# hand-written post and is tracked normally.",
        ".gitignore",
        *sorted(generated),
        "",
    ]
    (posts_dir / ".gitignore").write_text("\n".join(lines), encoding="utf-8")
...
def main() -> int:
    ...
    write_generated_gitignore(posts_dir, generated)
